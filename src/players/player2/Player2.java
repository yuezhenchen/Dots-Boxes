...
public class Player2 implements Player {
  ...
  private boolean isBox(int value, Board board){//63
    ...
    if (list.size() == 1){
      ...
      if(findSquare(nei,board).openLines().size()>2) {return true;}
      return false;
    }
    else {//list.size ==2
  ...
  public Line makePlay(Board board, Line oppPlay, long timeRemaining) {
    ...
    if(checkset.size()==0) {
      ...
      if(opendoubles.size() >0){
        ...
        Set<Square> loopSquare = board.squaresWithMarkedSides(2);
        Square loopsquare = loopSquare.iterator().next();
        Line line = loopsquare.openLines().iterator().next();
        return line;
      }
    }
    ...
    if (checkset.size() == 1) {
      Set<Integer> single = singlebox(board);
      Set<Integer> opendoubles = opendoubleset(board);//有问题
...
//      
  ...
  public String teamName() { return "CSCI1102STAFF"; }
